...
function getRandomSpeed() {
  return Math.random() * 2 + 1; // Adjust speed range if needed
}
...
function moveObject(object) {
  ...
  function animate() {
    ...
    // Keep the object inside the container
    if (posX < 0 || posX > container.clientWidth - 50) {
      speedX = -speedX; // Reverse direction if it hits a wall
      posX = Math.max(0, Math.min(posX, container.clientWidth - 50));
    }

    if (posY < 0 || posY > container.clientHeight - 50) {
      speedY = -speedY; // Reverse direction if it hits a wall
  ...
  animate();
  changeDirection(); // Start changing directions at random intervals
}

// Function to create a clickable object with embedded SVG
function createObject() {
  ...
  // Add SVG code to the object
  object.innerHTML = svgCode;

  // Make the object disappear on click and update the counter
  object.addEventListener('click', () => {
    object.remove();
    clickCount += 1; // Increase the click count
    counter.textContent = `Clicked: ${clickCount}`; // Update the counter display

    if (clickCount === 10) {
      displayWinMessage(); // Display win message after 10 clicks
    }
  });
